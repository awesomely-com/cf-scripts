...
class KeapFunnelHandler {
  ...
  setupOrderButtonListener(): void {
    ...
    const orderButtonText = document.querySelector(
      `${this.config.selectors.orderButton}>.elButtonMain`
    );
    if (orderButtonText instanceof HTMLElement) {
      orderButtonText.innerText = "Click to Continue";
    }
  ...
  async handleOrderResponse(response: any): Promise<void> {
    ...
    if (response.step?.next_step_url) {
      ...
      if (orderButton) {
        this.handleOrderError(
          new Error(
            response.message + ` - Please review your details and try again.` ||
